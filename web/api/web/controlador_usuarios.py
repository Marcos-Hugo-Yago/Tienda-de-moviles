...
def login_usuario(username, passwordIn):
    try:
        ...
        app.logger.error("Excepcion al validar al usuario: %s", e)
        ret = {"status": "ERROR"}
        code = 500
    return ret, code
...
def alta_usuario(username, password, correo):
    try:
        ...
        app.logger.error("Excepcion al registrar al usuario: %s", e)
        ret = {"status": "ERROR"}
        code = 500
    return ret, code
...
def logout():
    try:
        ...
        app.logger.error("Excepcion al cerrar sesion: %s", e)
        ret = {"status": "ERROR"}
        code = 500
    return ret, code
